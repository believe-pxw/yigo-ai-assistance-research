...
完整 BNF 定义：[peixw.bnf](file:///d:/Workbench/idea/yigo-ai-assistance-research/resource/peixw.bnf)

## XML 实体转义 ⚠️

在 XML **属性值**中（如 `Visible`、`Enable`），不能使用 CDATA，因此必须对特殊字符进行实体转义：
...
在 CDATA 内部（如 `<Action>`、`<Macro>` 等元素体内），**不需要转义**，可直接使用 `&&`、`<`、`>` 等。

### 示例对比

```xml
<!-- 属性中必须转义 -->
<NumberEditor Enable="!ReadOnly()&amp;&amp;Enable&lt;0"/>

<!-- CDATA 中无需转义 -->
<CheckRule><![CDATA[IIF(0<=CallHorizon&&CallHorizon<=100,true,MessageFacade('IP814','请输入0-100间的整数'))]]></CheckRule>
```
...
| `ToString(value)` | 转字符串 |
| `ToLong(value)` | 转长整数 |
| `GetEntryPara(paraName)` | 获取入口参数 |
| `MessageFacade(code, text)` | 弹出消息 |
| `SetEnable(fieldKey, bool)` | 设置控件可用性 |
| `SetVisible(fieldKey, bool)` | 设置控件可见性 |
